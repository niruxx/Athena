...
- 🧑‍💻 **User Management** — a dropdown group of its own for housekeeping that has nothing to do with packages: **General** (clear `/tmp` and `~/.cache`) and **Advanced** (trash, thumbnail cache, shell history, recently-used list) cleanup tabs with sizes shown before you delete anything, plus a **Backup & Restore** tab that archives your dotfiles/config (`.tar.gz` or a plain uncompressed copy, your choice) and restores them the same way — handy when moving to a new machine or OS.
